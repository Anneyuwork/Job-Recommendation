...
public class Recommendation {
	...
	public List<Item> recommendItems(String userId, double lat, double lon) {
		...
		// Step 1, get all favorited itemids for this user
		MySQLConnection connection = new MySQLConnection();
		Set<String> favoritedItemIds = connection.getFavoriteItemIds(userId);

		// Step 2, get all keywords, sort by count
		// allKeywords is get form iterating all the keywords for this user
		...
		//we want to get top 3 keyword, we use sort
		//list of map entry
		...
		for (Entry<String, Integer> keyword : keywordList) {
			...
			//check to filter visited or favorited items
			for (Item item : items) {
